...
已通过官方 riscv-tests 42 项全量测试，支持 4-way 组相联写回缓存，实测指令吞吐与缓存命中数据详见 [docs/PERFORMANCE_REPORT.md](docs/PERFORMANCE_REPORT.md)。
...
cmake --build build -j
# 运行官方 rv32ui 全量用例（脚本会扫描 riscv-tests/isa/rv32ui-p-*）：
./test_all.sh

# 若需静默采集每个 ELF 的耗时与仿真器统计（CSV），可以使用 mycpu 的 -q/--quiet 并按需解析输出。
```
